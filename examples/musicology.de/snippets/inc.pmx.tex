...
\subsubsection{Das große Versprechen, \ldots}

Als nächstes geht es um \emph{PMX}\footcite[vgl.][\nopage wp]{CtanPmx2018a},
will sagen: um einen Präprozessor für MusiX\TeX\footcite[Vgl.][4ff]{CtanPmx2018a}.

Ein \emph{PMX}-Tutorial nennt MusiX\TeX\ eines der besten Programme für den
...
Den Ausweg aus diesem Dilemma -- so das genannte Tutorial -- biete \emph{PMX},
das als Präprozessor den Eingabeprozess dramatisch vereinfache und so etwas
liefere, was zu den einfachsten Möglichkeiten gehöre, Notenblätter elektronisch
zu erarbeiten\footcite[vgl][3]{Noack2013a}. Die Gegenüberstellung der
...
./
% BODY: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
HEADER: a smaller width than the line width
w80m
% MUSIC: %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
r8 
\zt{-7}{D7}\ e44 za zc 
\zt{-7}{D7}\ e84 za zc r8                    |/
% T5: -----------------------------------------
r8 [l f83 a8- ] r8 b82l                      |/
r8 
\zt{-7}{D7}\ [u c85 za ze c85 za ze ] 
r8 
\zt{-7}{T}\ b84u zf zd                       |/
% T6: -----------------------------------------
[l f83 c83 f82 ] b42l                        |/
\zt{-7}{D7}\ [u c85 za ze c85 za ze c85 za ze ] 
\zt{-7}{T}\ b44u zf zd                       |/
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% EOF
\end{verbatim}
...
Trotz der so euphorisch kolportierten Vorteile wächst sich \emph{PMX} bei
...
Das liegt in erster Linie an der Arbeitsweise von \emph{PMX}\footnote{In zweiter
Linie liegt es an den Entwicklern von \emph{PMX}. Sie haben schlicht (noch) kein
korrespondierendes \LaTeX-Paket geschaffen. Technisch mag das herausfordernd
sein, unmöglich ist es nicht: Es wird ja schon gesagt, dass der von \emph{pmxab}
...
entsprechende Tool \emph{pmxab} eine \emph{PMX}-Datei und erzeugt daraus eine
...
Die eine, 'direkte' Lösung für dieses Problem besteht darin, zuerst -- ganz
\LaTeX-unabhängig -- aus der \emph{PMX}-Datei eine Graphik zu erzeugen
und diese anschließend in die \LaTeX-Datei einzubinden\footcite[Vgl. dazu
auch][94]{Noack2013a}:

\begin{itemize}
  \item Dazu ruft man für eine \emph{PMX}-Datei zunächst den
  \emph{PMX}-Präprozessor auf (z.B. \texttt{pmxab candeza1.pmx}), um die
  korrespondierende MusiX\TeX-Datei \texttt{candeza1.tex} zu genieren.
  \item Das Resultat übergibt man dann dem \emph{MusiX\TeX}-Tool als Input, und
  zwar mit der Option \emph{-g}: (\texttt{musixtex -g candeza1.tex}). So wird
  nicht eine \emph{PDF}-Datei erzeugt, sondern die Postscriptdatei
  \texttt{candeza1.ps}.
  ...
  \emph{eps}-Graphik erscheinen soll, den \LaTeX-Befehl 
  \texttt{\textbackslash{includegraphics}\{candeza1.eps\}}
  ein\footnote{Hier muss man beachten, an der Stelle, von wo aus man die
  \emph{eps}-Datei einlesen will, nicht (versehentlich) auch eine 'gleichnamige'
  \emph{pdf}-Datei abgelegt zu haben. In diesem Fall lädt
...
erstellt hat, warum sollte man sich dann die Mühe antun, zwecks
Erstellung einer Graphik zuerst \emph{PMX} zu lernen, wo es doch so viele gute
Notenprogramme gibt, die ihre Daten auch als Postscript- oder PDF-Datei
exportieren? Das könnte doch nur dann sinnvoll sein, wenn man wenigstens auf die
...
Autor des \emph{PMX}-Tutorials sehr wohl um die Irritationen in Sachen \emph{PMX
/ MusiX\TeX\ } und \emph{\LaTeX\ } weiß, ja mehr noch, dass er sie auch nicht
...
Notenblätter zu erzeugen. Und für diesen Zweck ist die Lösung \emph{PMX
...
Kadenz-III in der \emph{PMX}-Variante\footnote{$\rightarrow$ S.
\pageref{\cadlab{III}{PMX}}} wirkt optisch weniger klar als die
\emph{MusiX\TeX}-Variante\footnote{$\rightarrow$ S.
...
die \emph{PMX} -- sozusagen nach Schema F -- automatisch trifft, können solchen
fallweisen Feinheiten natürlich nicht wirklich gerecht werden. Diese kann man
gelegentlich allerdings auch für reine Geschmacksfragen halten.

Die zweite Unzulänglichkeit betrifft die Methode, wie die Analysesymbole in den
\emph{PMX}-Code eingettet werden: \emph{PMX} bietet zwar die Möglichkeit, Stücke
auf \emph{PMX}-Level textuell zu kennzeichnen oder Texte über oder unter den
...
einzufügen, dann muss der \emph{PMX}-User auf \enquote{Inline \TeX\ commands}
zurückgreifen, also auf den \TeX-Level
'zurückfallen'\footcite[vgl.][76f]{Noack2013a}: die Nutzung einer vereinfachten
\emph{PMX}-Syntax wird dann um den Preis einer komplizierten Texteingabe
erkauft\footnote{Weil \TeX\ 'zu' kompliziert war, wurde \LaTeX\ 'erfunden';
und weil \emph{PMX} mit Text 'zu' kompliziert war, entstand \emph{M-Tx}
(\cite[vgl.][\nopage wp]{CtanMtx2018a}) -- einschließlich eines
Handbuches (\cite[vgl.][3ff]{Laurie2017a})}.

Wirklich unzulänglich ist dabei jedoch das, \emph{was} eingebettet werden kann.
...
\emph{harmony}-Paket zur Verfügung stellt. Denn das Konvertierungstool
\emph{pmxab}, das \emph{MusiX\TeX}-Code aus \emph{PMX}-Code ableitet, arbeitet
ja noch außerhalb von \TeX. Und das Konvertierungsttool \emph{musixtex}
versteht 'nur' \emph{Musix\TeX}-Code, nicht aber \LaTeX-Syntagmen.

\subsubsection{\ldots und die kleine Lösung}
...
inkludiertem MusiX\TeX-Package} auf \emph{PMX} zurückzugreifen: Wenn es nämlich
...
Eine Lösung dafür bestünde darin, zuerst den reinen Notentext mit \emph{PMX} zu
erstellen und dann mit \emph{pmxab} in MusiX\TeX-Code umzuwandeln. Danach griffe
...
\emph{\textbackslash{begin}\ldots\textbackslash{end}\{music\}}-\LaTeX-Umgebung
kopiert werden kann\footcite[vgl.][94]{Noack2013a}. Dieses Verfahren wollen wir
noch kurz vorführen:

Wir wissen ja schon, dass wir die Symbole der Harmonieanalyse, die wir auf
\emph{PMX}-Level erstellen könnten, bei einer Einbettung in einen \LaTeX-Code
werden nicht verwenden können, eben weil es ja keine \LaTeX-Konstrukte sind,
sondern \TeX-Syntagmen. Deshalb notieren wir diesmal auf \emph{PMX}-Level nur
...
Ausgegangen wird dabei von folgendem \emph{PMX}-Code:
...
% T:1-1     T:1-2    T:1-3                T:1-4
  d23l za+  b-l zg+  a-l zg+              dl  zf |/
  f24u zd+  g-u zd+  cu  ze               a-u zd |/
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% T:2-1     T:2-2    T:2-3                T:2-4
  d23l za+  g  zb    a zc                 d- za+  /
  d25u zf   g- zd+   ( e4- zd+ e4- ) zc+  f2- zd+ |/
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  EOF
\end{verbatim}

Lässt man diesen \emph{PMX}-Code von \texttt{pmxab} verarbeiten, entsteht ein
Musix\TeX-Code, bei dem man, will man ihn in die Musix\TeX-in-\LaTeX-Umgebung
\verb|\begin{music}...\end{music}| einbetten, noch  einige Zeilen auskommentieren und
einige Werte ändern muss:
...
\normalmusicsize %%% KR instead of: %%% \smallmusicsize%
%%ignore:%% \nopagenumbers
...
\parindent 4em %%% KR instead of %%% 37pt
...
Vor einer erfolgreichen Nutzung diese Codes muss man allerdings in seinem
\LaTeX-Header auch noch die Regel \verb|\newcommand{\pnotes}[1]{\notes}|
formuliert haben muss, damit aus der Einbettung auf \LaTeX/MusiX\TeX-Ebene --
also ohne Einbindung einer externen Graphik -- das folgende Bild  entsteht:
...
Und eben dahin kann man nun auch wieder die \LaTeX-basierten Elemente einer
Harmonieanalyse einbaue \ldots
...
\notes
  ...
  \zmidstaff{\HH.S.3....}\zh G\hl B%
  \zmidstaff{\HH.D.7....}\zh G\hl A%
  \zmidstaff{\HH.T.....}\zh F\hl D|%
  \zh{'d}\hu{`f}\zh{'d}\hu{`g}\zh{'e}\hu c\zh d\hu a%
\en%
% Bar count 2
\xbar
\notes
  \zmidstaff{\HH.T.....}\zh{''A}\hl{`D}%
  \zmidstaff{\HH.S.....}\zh{'B}\hl{`G}|%
  \zh{'f}\hu d\zh d\hu{`g}%
\en%
\notes
  \zmidstaff{\HH.D...4-3.5.}\zh{''C}\hl A|%
  \islurd0e\zq{'d}\qu{`e}\tslur0e\zq{'c}\qu{`e}%
\en%
\notes
  \zmidstaff{\HH.T.....}\zh{''A}\hu{`D}|%
  \zh{'d}\hu{`f}\en%
\Endpiece
\end{music}%
\cad{II}{PMX+MusiXTeX-in-LaTeX++}
...
wobei -- zugegebenermaßen -- Geduld, ein geschulter Blick und der Wille zum
Codeclearing hilfreich sind:
