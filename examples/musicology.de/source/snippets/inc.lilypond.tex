...
Druck unterscheidet: Wer in D-Dur ein \textit{fis} einfügen möchte, kann sich hier
...
automatisch setzen. In \textit{g-moll} erhält das \textit{fis} ein Kreuz, in D-Dur
...
von \textit{LilyPond} als solches sein wird, sondern die Einbindung jener Symbole in
...
oder die \textit{harmony}-Konstrukte wird zurückgreifen können: \textit{LilyPond}
arbeitet \LaTeX-unabhängig.

\textit{LilyPond} bietet 3 Verfahren an, (echten) Text in die Noten zu integrieren:
...
\textit{LilyPond} wird für den Musikwissenschaftler also sein, ob und wie gut man
die Symbole der Harmonieanalyse, die \textit{harmony} bereitstellt, mit den
Mitteln von \textit{LilyPond} nachbilden kann und welcher Aufwand dabei entsteht.
...
Wir haben deshalb begonnen, eine kleine \textit{LilyPond}-Library zu entwickeln,
die man einfach in seine \textit{LilyPond}-Datei mit dem Befehl
...
dass die \textit{LilyPond}-Noten irgendwie 'weicher' und 'dichter' sind:
\textit{LilyPond} hat seine selbst gesetzte Aufgabe offensichtlich
