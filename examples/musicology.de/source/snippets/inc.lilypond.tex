...
\emph{lilypond}-Sektionen enthält, und erzeugt die entsprechenden Graphiken,
bevor es die \emph{lilypond}-Sektionen durch die korrespondierenden
...
von \emph{LilyPond} als solches sein wird, sondern die Einbindung jener Symbole in
den Notentext, die für den Musikwissenschaftler so wichtig sind. Und es ist an
dieser Stelle auch gewiss, dass man dabei nicht auf die \LaTeX-Sonderzeichen
oder die \emph{harmony}-Konstrukte wird zurückgreifen können: \emph{LilyPond}
...
Die letztlich entscheidende Frage hinsichtlich der Verwendbarkeit von
\emph{LilyPond} wird für den Musikwissenschaftler also sein, ob und wie gut man
die Symbole der Harmonieanalyse, die \emph{harmony} bereitstellt, mit den
Mitteln von \emph{LilyPond} nachbilden kann und welcher Aufwand dabei entsteht.
...
\emph{harmony} nachzubilden, artet jedoch in echte Programmierarbeit aus und
dürfte einem Musikwissenschaftler kaum mehr zuzumiten sein - wohl aber denen,
...
\texttt{\textbackslash{include}} hinzulädt. Danach kann man die entsprechende
Symbole der Harmonieanalyse in seinen Notentext hinzufügen. Unsere aktuelle
stabile Version liefert folgendes -- sicher noch nicht ganz optimales --
Ergebnis\footnote{Die Einschränkungen werden aber nicht so bleiben! Z.Zt.
benötigen wir noch \ldots
\begin{itemize}
  \item die Möglichkeit, innerhalb des \emph{LilyPond}-Markups Zeichen
  ...
  \item die \emph{Guile-} resp. \emph{LilyPond}-Techniken zum Refactoring,
...
Und so fehlt noch die dritte Kadenz in der Version, die \emph{LilyPond} heute -- also
noch mit einigen Einschränkungen -- erzeugen kann:
...
Vergleicht man das 'Druckbild' von Noten, die \emph{LilyPond} erzeugt, mit
denen, die MusiX\TeX\ im Verbund mit \LaTeX\ generiert, dann fällt in der Tat auf,
dass die \emph{LilyPond}-Noten irgendwie 'weicher' und 'dichter' sind:
\emph{LilyPond} hat seine selbst gesetzte Aufgabe offensichtlich
erfüllt\footcite[vgl.][8ff]{LilyPond2018d}.

Musikwissenschaftler werden gut mit \emph{LilyPond} 'klarkommen': diese
Notationsweise zu lernen, ist einfach, sie anzuwenden leicht. In dieser Hinsicht
bietet \emph{LilyPond} mehr als MusiX\TeX. Wenn es allerdings um die Integration
von Symbolen der Harmonieanalyse in den Notentext geht, so liefert die
Verbindung von \LaTeX, MusiX\TeX\ und das Paket \emph{harmony} \underline{noch} die
